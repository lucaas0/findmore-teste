...
const OrcamentosVendedor = ({ idVendedor }) => {
    ...
    useEffect(() => {
		if (filtro === 'analise') {
			setOrcamentosFiltrados(orcamentos.filter((o) => o.estado === 'analise'));
		} 

		if (filtro === 'aceite') {
			setOrcamentosFiltrados(orcamentos.filter((o) => o.estado === 'aceite'));
		} 

		if (filtro === 'concluido') {
			setOrcamentosFiltrados(orcamentos.filter((o) => o.estado === 'concluido'));
		} 

		if (filtro === 'expirado') {
			setOrcamentosFiltrados(orcamentos.filter((o) => o.estado === 'expirado'));
		} 

		if (filtro === 'rejeitado') {
			setOrcamentosFiltrados(orcamentos.filter((o) => o.estado === 'rejeitado'));
		} 
	...
	const getOrcamentos = async () => {
		try {
			const { data } = await axios.get(
				`http://localhost:3001/orcamento/vendedor/${idVendedor}`
			);
	...
	return (
		<div style={{ display: 'flex', flexDirection: 'column', gap: '30px' }}>
            ...
            <div className="filtros-chips">
				<button onClick={() => handleFiltroClick('analise')} className={filtro === 'analise' ? 'selected' : ''}>
					Análise
				</button>
				<button onClick={() => handleFiltroClick('aceite')} className={filtro === 'aceite' ? 'selected' : ''}>
					Aceite
				</button>
				<button onClick={() => handleFiltroClick('concluido')} className={filtro === 'concluido' ? 'selected' : ''}>
                    Concluído
				</button>
				<button onClick={() => handleFiltroClick('expirado')} className={filtro === 'expirado' ? 'selected' : ''}>
					Expirado
				</button>
				<button onClick={() => handleFiltroClick('rejeitado')} className={filtro === 'rejeitado' ? 'selected' : ''}>
					Rejeitado
				</button>
			</div>

			{orcamentosFiltrados.length === 0 && <h1 style={{ fontSize: '22px' }}>Não existem orçamentos.</h1>}
...
export default OrcamentosVendedor;
