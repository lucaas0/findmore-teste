...
export default function PagamentoPacote({
	onLogout,
	pacote,
	quantity,
	onCancelar,
	...
	const handlePagar = async () => {
		try {
			await axios.post(`http://localhost:3001/compra/pacote`, {
				idUtilizador: authService.getCurrentUser().idUtilizador,
				idPacote: pacote?.idPacote,
				valorTotal: pacote?.valor * quantity,
			...
			handleShowToast(
				error.response.data.error ||
					'Ocorreu um erro ao concluir a compra!',
				'danger'
			);
			// setShowDialog(false);
		}
	...
	const updateOrcamento = async () => {
		if (pacote.idOrcamento) {
			try {
				const res = await axios.put(
					`http://localhost:3001/orcamento/${pacote.idOrcamento}`,
					{
						estado: 'concluido',
					}
				);
				...
				<div className='buy-wrapper'>
					...
					>
						<div className='buy-wrapper-footer-resumo'>
							...
							<div>
								<span>Total: </span>
								<span>{`${(pacote?.valor * quantity).toFixed(
									2
								)}€`}</span>
							</div>
						</div>
